...
槓桿報表目前是美股 ETF margin loan 研究模型，預設 1.3x 目標槓桿、6.5% 年化借款利率、35% 維持率、安全緩衝低於 25 percentage points 時自動降到 1.1x。它會使用 raw price，再把股息、30% 美股股息預扣稅、現金股息或股息再投入接進同一個 margin ledger，避免 adjusted price 與股息重複計算。
