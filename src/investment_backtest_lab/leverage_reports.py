...
def _render_html(
    *,
    ...
    compare_lab = _render_leverage_compare_lab(metrics, curves)
...
def _render_leverage_compare_lab(metrics: pd.DataFrame, curves: pd.DataFrame) -> str:
    payload = _build_leverage_compare_payload(metrics, curves)
    payload_json = _json_for_script(payload)
    checkboxes = _render_compare_checkboxes(payload["scenarios"], lab_id="leverage")
    ...
    data-compare-lab="leverage"
    data-compare-payload="leverage-compare-payload">
  <div class="section-heading">
    <div>
      <p class="eyebrow">Compare Lab</p>
      <h2>槓桿 Compare Lab</h2>
    </div>
    <p>自由勾選想疊圖比較的情境，再切換指標。建議最多 6 條線；超過仍可畫，但可讀性會下降。</p>
  </div>
  <div class="notice-card warning-notice">
    <strong>DCA 與標準化曲線提醒</strong>
    <p>DCA 有外部現金流，normalized equity 只適合看路徑形狀，不能當作實際本金報酬排名。</p>
  </div>
  <div class="compare-layout">
    ...
    <div class="compare-main">
      ...
      <div id="leverage-compare-chart" class="compare-chart" data-compare-chart></div>
    </div>
  </div>
  <script type="application/json" id="leverage-compare-payload">{payload_json}</script>
</section>"""


def _build_leverage_compare_payload(metrics: pd.DataFrame, curves: pd.DataFrame) -> dict[str, Any]:
    metric_config = {
        "total_equity": {"label": "權益曲線", "axis": "USD", "format": "money"},
        "normalized_equity": {"label": "標準化權益", "axis": "Normalized USD", "format": "money"},
        "drawdown": {"label": "回撤", "axis": "Drawdown", "format": "percent"},
        "debt": {"label": "負債", "axis": "Debt USD", "format": "money"},
        "safety_buffer": {"label": "安全緩衝", "axis": "Safety Buffer", "format": "percent"},
        "actual_leverage": {"label": "實際槓桿", "axis": "Leverage", "format": "leverage"},
        "target_leverage": {"label": "目標槓桿", "axis": "Leverage", "format": "leverage"},
    }
    default_keys = {
        _scenario_key("SPY_QQQ", "rebalance_leveraged", "cash"),
        _scenario_key("SPY_QQQ", "dynamic_rebalance_leveraged", "cash"),
    }
    metric_lookup = {
        _scenario_key(row.ticker, row.strategy, row.dividend_mode): row
        for row in metrics.itertuples()
    }
    scenarios: list[dict[str, Any]] = []
    for key_tuple, group in _iter_curve_groups(curves):
        ...
        series = {
            ...
            "debt": _json_series(group.get("debt")),
            "safety_buffer": _json_series(group.get("safety_buffer")),
            "actual_leverage": _json_series(group.get("actual_leverage")),
            "target_leverage": _json_series(group.get("target_leverage")),
        }
        metric_row = metric_lookup.get(key)
        subtitle = ""
        if metric_row is not None:
            subtitle = _scenario_button_subtitle(metric_row)
        scenarios.append(
            {
                "key": key,
...
def _render_compare_checkboxes(scenarios: list[dict[str, Any]], *, lab_id: str) -> str:
    if not scenarios:
        return '<p class="empty-state">沒有可比較的情境。</p>'
    checkboxes: list[str] = []
...
def _json_series(values: Any) -> list[float | None]:
    ...
    for value in series:
        if pd.isna(value) or value in (inf, -inf):
            output.append(None)
        else:
